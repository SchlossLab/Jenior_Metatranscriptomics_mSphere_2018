...
strep_genus_01_richness <- as.numeric(length(strep_genus_diff_01$transcriptChange)) /
  (as.numeric(length(strep_genus_diff$transcriptChange)) - as.numeric(length(strep_genus_diff_01$transcriptChange)))
strep_genus_01_richness <- round(strep_genus_01_richness * 100, 2)

cef_genus_01_richness <- as.numeric(length(cef_genus_diff_01$transcriptChange)) /
  (as.numeric(length(cef_genus_diff$transcriptChange)) - as.numeric(length(cef_genus_diff_01$transcriptChange)))
cef_genus_01_richness <- round(cef_genus_01_richness * 100, 2)

clinda_genus_01_richness <- as.numeric(length(clinda_genus_diff_01$transcriptChange)) /
  (as.numeric(length(clinda_genus_diff$transcriptChange)) - as.numeric(length(clinda_genus_diff_01$transcriptChange)))
clinda_genus_01_richness <- round(clinda_genus_01_richness * 100, 2)

genus_01_richness <- median(c(strep_genus_01_richness, cef_genus_01_richness, clinda_genus_01_richness))
genus_01_richness <- as.character(genus_01_richness)
...
**Statistical methods.** All statistical analyses were performed using R (v.3.2.0) and the vegan package [@vegan2018]. Significant differences of inverse Simpson diversity, cfu, toxin titer, and metabolite concentrations were determined by Wilcoxon signed-rank test with Benjamini-Hochberg correction using a study-wide Type I error rate of 0.05. Undetectable points used half the limit of detection for cfu and toxin statistical calculations. Random forest was performed using the AUCRF implementation [@Calle2011] as well as the standard package [@Breiman2001] in R. Distances of outlier points from center line during metatranscriptomic comparisons was accomplished using 2-dimensional linear geometry.
