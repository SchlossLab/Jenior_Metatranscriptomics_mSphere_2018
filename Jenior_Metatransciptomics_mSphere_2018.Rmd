...
av_noabx_pval <- round(p.adjust(av_noabx_pval, method='BH'), 3)


av_noabx_mock_pval <- as.character(max(c(av_noabx_pval[2], av_noabx_pval[4], av_noabx_pval[6])))
...
```{r figure_s2d, echo=F, warning=F, message=F, results='hide', cache=F}
...
amv_pval <- as.character(amv_pval)
rm(aminovalerate)

```

***C. difficile* colonization induced distinct metabolomic changes of each pretreatment group.**
Following the changes to the cecal metabolome in response to antibiotic pretreatment, we assessed the degree to which *C. difficile* colonization altered the cecal metabolome. We hypothesized that the introduction of a new competitor, *C. difficile*, would impact the metabolome either through its own metabolism or by shifting the metabolism of surrounding community members. First, we compared the dissimilarity of cecal metabolomes from all mice across pretreatment groups, and were unable to detect a consistent difference between groups (*p* = `r inf_p`; Fig. S3A). We moved on to individual antibiotic groups and found that both streptomycin (*p* = `r strep_inf_p`) and cefoperazone (*p* = `r cef_inf_p`) pretreated metabolomes deviated significantly from that of mock infection (Fig. 4A-4B). However, clindamycin-pretreated cecal metabolomes were not significantly altered by the presence of the pathogen (*p* = `r clinda_inf_p`; Fig. 4C). These results diverged from what was seen in the paired OTU relative abundance results where instead the cecal community structure of mice pretreated with cefoperazone was unchanged and clindamycin-pretreated communities were significantly different (Fig. S1). Interestingly, streptomycin-pretreated microbiomes were significantly altered by *C. difficile* infection at both the OTU and metabolomic levels (Fig. 4A & S1D). Together, these data indicated that shifts in population within the cecal microbiota were not implicitly associated with concordant changes in the metabolome. 



We then sought to identify changes in those metabolites that were potential markers of *C. difficile* infection through a unified analysis across all models tested. To accomplish this, we again applied the Random Forest machine-learning approach. The resultant models were able to reliably classify infected and uninfected mice in both streptomycin and cefoperazone pretreatments (Fig. 4D-E, Out-of-bag error = 0%). 




ectoine helps bacteria with osmotic stress []...(Fig. 4E).





However, infection did not provide for the same level of discrimination in clindamycin-pretre45ted mice (Fig. 4F, Out-of-bag error = 5.56%). 

While these data supported our hypothesis that each antibiotic pretreatment also drove the generation of unique metabolic environments, it was not possible to attribute many other specific changes to altered microbiota or host activity.


Interestingly, only 5-aminovalerate appeared multiple times as an optimal discriminatory metabolite across pretreatments. This molecule is a known byproduct of D-proline Stickland fermentation by *C. difficile* [@Bouillaut2013;@Neumann-Schaal2015]. Presence of 5-aminovalerate appears to be dependent on the metabolism of *C. difficile* as concentrations were significantly lower in antibiotic-pretreated, mock-infected animals (*p* < `r av_noabx_mock_pval`; Fig. 4G). Conversely, it was highly abundant in most infected animals compared to both susceptible and resistant controls. Furthermore, 5-aminovalerate only became detectable in the cecum of germfree mice after infection by *C. difficile* str. 630 (*p* < `amv_pval`; Fig. S2D). However, exclusively in clindamycin-pretreatment where CDI was quickly became undetectable, did the concentration of 5-aminovalerate remain near undetectable in a subset of samples. *C. difficile* is able to catabolize proline along with glycine through a set of paired biochemical reactions known as Stickland fermentation. Along these lines, proline and 3 proline-conjugated amino acids were all significantly decreased during infection (*p* < `r infection_pvalues`; Fig. S3B). These combined results demonstrated that while distinct metabolic challenges may exist in each susceptible metabolome, Stickland fermentation could be a preferred energy acquisition pathway for *C. difficile* *in vivo*. 
...
**Figure 1 | Differential impact of respective antibiotic pretreatments on sustained *C. difficile* colonization and the microbiota.** **(A)** *C. difficile* 630 cfu in stool of infected mice following each antibiotic-pretreated group over 10 days of infection. Median and interquartile range are shown for each time point. Both cefoperzone and streptomycin pretreatments had more significantly detectable cfu on the final day of detectable cfu associated with clindamycin-pretreatment (*p* < 0.001). **(B)** Experimental time lines for subsequent experimentation for antibiotic pretreatments and 18-hour infection mouse models. Nine mice were used in each experimental and control group and samples from these animals were used for all downstream analyses. **(C)** Quantification of terminal vegetative *C. difficile* cfu in cecal content across 18 hour colonization models. Black lines indicate median values and each pretreatment group had significantly greater detectable cfu than no antibiotic controls. Significant differences in A & C were determined by Wilcoxon rank-sum test with Benjamni-Hochberg correction when necessary. The limit of detection was used in place of undetectable values for statistical testing. **(D)** Relative abundance of family-level taxonomic classification for OTUs in each pretreatment group. 


**Figure 2 | Larger degree of conserved signal for susceptibility in metabolome compared to community structure.**
