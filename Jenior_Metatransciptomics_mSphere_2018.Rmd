...
No duh, there's more minority than majority taxa overall, how to reconcile that...

We further characterized this by tabulating the absolute difference between mock-infection and CDI in all transcriptomic abundances per genus in each antibiotic pretreatment, normalizing for the number of genes detected per each genera. Taxa were then stratified into catagories based on their relative abundance in each community from 16S rRNA gene sequencing (Fig. 4). This revealed that most change occurred within relatively rare bacterial taxa. The diversity of taxa in the lowest abundance bracket was significantly larger than that of all other abundance groups combined (*p* < `r `), so as a corrollary the majority of unique genes and metabolic potential was held within this minority. Consequently, the differences in activity noted here may affect a disproportionately large change in the overall environment of the intestine as a function of their collective metabolism. 
...
Based on the metabolomic results we hypothesized that pathways with the greatest differences would be related to catabolism of metabolites in established *C. difficile* nutrient niches.
