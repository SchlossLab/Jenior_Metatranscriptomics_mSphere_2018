...
# Output plot name
plot_ab <- 'results/figures/figure_2ab.pdf'
metabolome_plot <- 'results/figures/metabolome_fig2.pdf'
otu_plot <- 'results/figures/otu_fig2.pdf'
...
# Metabolome
metabolome_thetayc <- designdist(metabolome[,2:ncol(metabolome)], method='1-(J/(A+B-J))', terms='quadratic', abcd=FALSE)
metabolome_nmds <- as.data.frame(metaMDS(metabolome_thetayc, k=2, trymax=100)$points)
...
# permANOVA
metabolome_pval <- adonis(metabolome_thetayc ~ metabolome$susceptibility, metabolome, perm=999)$aov.tab
...
# 16S
otu_thetayc <- designdist(shared_otu[,2:ncol(shared_otu)], method='1-(J/(A+B-J))', terms='quadratic', abcd=FALSE)
otu_nmds <- as.data.frame(metaMDS(otu_thetayc, k=2, trymax=100)$points)
...
# permANOVA
otu_pval <- adonis(otu_thetayc ~ shared_otu$susceptibility, shared_otu, perm=999)$aov.tab
otu_pval <- round(otu_pval[1,6], 3)
# Average within group distances
...
# 16S
# Get and BLAST sequences you lazy fuck!


colnames(res_shared_otu) <- c("Lachnospiraceae (OTU42)","Clostridia unclassified (OTU102)","Lachnospiraceae (OTU111)","Clostridiales unclassified (OTU115)","Lachnospiraceae (OTU128)")
colnames(sus_shared_otu) <- c("Lachnospiraceae (OTU42)","Clostridia unclassified (OTU102)","Lachnospiraceae (OTU111)","Clostridiales unclassified (OTU115)","Lachnospiraceae (OTU128)")
...
# 16S
plot(x=otu_nmds$MDS1, y=otu_nmds$MDS2, xlim=c(-1.9,1.9), ylim=c(-2.5,2.5),
...
points(x=otu_nmds_susceptible$MDS1, y=otu_nmds_susceptible$MDS2, bg='forestgreen', pch=21, cex=2, lwd=1.2)
...
points(x=metabolome_nmds_susceptible$MDS1, y=metabolome_nmds_susceptible$MDS2, bg='forestgreen', pch=21, cex=2, lwd=1.2)
...
# 16S
metabolite_stripchart(otu_plot, res_shared_otu, sus_shared_otu, shared_otu_pval,  
                      shared_otu_aucrf_oob, 'Resistant', 'Susceptible', noabx_col, 'forestgreen', '', 'black')
# Metabolome
metabolite_stripchart(metabolome_plot, res_metabolome, sus_metabolome, metabolome_pval,  
                      metabolome_aucrf_oob, 'Resistant', 'Susceptible', noabx_col, 'forestgreen', '', 'black')
