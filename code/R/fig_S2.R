...
metabolome <- metabolome[, colnames(metabolome) %in% bile_acids]  
...
abx_pvalues1[is.na(abx_pvalues1)] <- 1
abx_pvalues2[is.na(abx_pvalues2)] <- 1
abx_pvalues3[is.na(abx_pvalues3)] <- 1

#-------------------------------------------------------------------------------------------------------------------------#

# Plot the figure
pdf(file=plot_file, width=4, height=ncol(strep_abx_metabolome)*1.5)
...
for(i in c(1:(ncol(strep_abx_metabolome)))){
  ...
  legend('topright', legend=colnames(strep_abx_metabolome)[i], pch=1, cex=1.3, pt.cex=0, bty='n')
