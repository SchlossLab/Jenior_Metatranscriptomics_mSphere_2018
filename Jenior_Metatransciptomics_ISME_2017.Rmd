...
We then moved on to a more fine-scale resolution analysis and quantified expression changes at the individual gene annotation level. Similar to the pathway-level analysis, transcription for gene homologs between species was pooled to find those genes with the greatest overall difference in expression between conditions. We then selected the genes in each antibiotic pretreatment group with the largest disparity in normalized cDNA abundance between mock and *C. difficile*-infected groups (Fig. 2). In agreement with pathway-level differences in expression, for streptomycin pretreatment the genes with ...
(Fig. 2b). 


(Fig. 2c). It important to note that these groups are equally subsampled, meaning that more genes are evenly expressed at lower levels in mock infection instead of a small number of genes highly upregulated. 


(Fig. 2d). These data expand on our interpretation of pathway-level analysis in that *C. difficile* colonization leads to 
changes in community-level expression of genes for acquisition and catabolism. Our results also raised questions about which taxonomic groups underscore the differences seen and are potentially more impacted by *C. difficile* colonization than others.
...
In light of large-scale differences in expression across the microbiota concomitant with *C. difficile* colonization, we desired to identify those bacterial groups responsible for the shift in each pretreatment condition. To accomplish this we utilized the genus level taxonomic information associated with each KEGG annotation to identify which group likely contributed a given gene to the metagenome. Many genes in the KEGG database are annotated as hypothetical or uncharacterized but still possess a taxonomic annotation. This resulted in substantially more genes from the total  being conserved for analysis in each group; 19.15% (streptomycin), 25.1% (cefoperazone), and 44.45% (clindamycin). With these data, we narrowed the focus onto transcription for genera that represented >0.01% of genes receiving taxonomic annotations in any of the three metagenomic assemblies. We then directly compared the normalized cDNA abundances for each gene between infected & uninfected states for each antibiotic pretreatment and colored points based on genus classifications (Fig. 3a-3c). Coordinates were determined by the relative expression of each gene in mock and *C. difficile* infection for x and y axes respectively. This causes genes with equal transcription in both conditions being compared to be strongly correlated and positioned proximal to the central diagonal line. As such, we then applied linear correlation and least-squares residual analysis to elucidate those genes that are most strongly upregulated in either infection condition.

These data revealed distinct changes in expression for underrepresented genera for each antibiotic pretreatment between infections conditions. First, in streptomycin-pretreated mice, the most prominent differences were in a large number ( GET NUMBER ) of genes belonging to *Lactobacillus* were upregulated in *C. difficile* infection (Fig. 3a). Next, in cefoperazone pretreatment primarily genes belonging to *Allistipes* were more upregulated in mock infection (Fig. 3b). A consistent trend in streptomycin and cefoperazone pretreatments was an overrepresentation of expression from genera belonging to Bacteroidetes during mock infection. Interestingly, taxonomic groups most highly represented in the normalized metatranscriptomes of either of these pretretreatments are actually minority members of the respective cecal communities as determined 16S rRNA gene abundance (Fig. 1c). Importantly, genes from highly abundant taxa in cefoperazone and streptomycin pretreatments are primarily contained in the highly correlated gray area, indicating very little change following *C. difficile* colonization (Table S5...). Finally, in clindamycin pretreatment transcription of *Escherichia* genes was upregulated in both mock and *C. difficile* infection (Fig. 3c). These data directly correlated with results from the corresponding 16S relative abundances where clindamycin-pretreated cecal communities were dominated by an *Escherichia* OTU (Fig. 1c). Correlation analysis of these metatranscriptomes also revealed that uninfected and infected conditions differed the least compared to the other pretreatmetns (*r* = 0.862). This infection model has been shown to go on to clear *C. difficile* infection witin 7 days, as opposed to long-term colonization, which suggests that communities where *C. difficile* is less able to impact resident members' activity regain colonization resistance more quickly (Fig. S5....). As a side note, the presence of Archeael sequences in our shotgun sequencing data was not surprising as the universal 16S rRNA gene V4 region primers we used do not reliably amplify Archeae. Together, our metatranscriptomic results support that *C. difficile* may differentially modify the transcriptional activity of microbial taxa based on the context of the community in which it is colonizing. These patterns could ultimately contribute to the ability of the pathogen to colonize differentially susceptible gut environments.
