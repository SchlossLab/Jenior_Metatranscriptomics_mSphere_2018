...
Using the previously mentioned metagenomic assembly, we clustered contigs into decreet groups based on composition and raw read coverage with CONCOCT [@]. We also performed additional heirarchical clustering implementing paired read information to generate contig clusters with the highest possible confidence. With these individual sequence groups, we identified and annotated genes in a similar fashion to was was done with the entire metagenome. This resulted in distinct collections of annotated genes that each resemble a bacterial genome. Due to the the unreliability of species-level identification of genes, we further groups clusters that shared a genus from which the majority of their gene composition was annotated. These resulting annotated pools of genes with highly similar taxonomic origin we treated as representative genomes for the entire genus. This enabled strict cDNA read mapping to the final clusters to acquire the transcriptional activity of each taxon separately in the complex community. Finally, using all of the combined gene and transcript information we accumulated through the aformentioned steps, we generated contig-enabled models (CEMs), similar to the genome-scale models in [@Jenior2017], for each to identify possible reporter metabolites of competition or cooperation between species in both mock and *C. difficile*-infected conditions across pretreatment groups. 
...
In previous work from our group [@Jenior2017], we established a simple computational framework to impute those metabolites which a bacterium is most likely to obtain from its environment based transcriptome-informed genome-enabled metabolic models. In the current study, we have extended these methods to analyze the putative competition for nutrients between bacteria in the perturbed cecal microbiota. This approach is especially informative when assessing which groups are most competitive with *C. difficile* for growth nutrients and may prevent its colonization in healthy communities. In this new method, the level of possible competition for a substrate is imputed based on the transcription of enzymes that utilize it within a pair of organisms, then this process is repeated iteratively for all shared metabolites between the pair. For each comparison, the minimum ratio for each shared positive metabolite importance score was calculated to derive a value that reflects the level of competition for a given metabolite between the two species.


This process then extended to each possible pairing of taxa in the community of interest in an effort to determine those metabolites that are highly sought after as well as identifying potentially vacant nutrient niches that *C. difficile* could exploit following infection.

automatically incorporates abundace of species do to the large volume of transcripts for each group, comparisons are at a population-level not at the individual
...
**Community-level metabolite scores from modeling analysis are predictive of cecal metabolite concentrations**

discuss supp figure here for metabolomics only


models included were pruned based on reads recruited, metabolite + enzyme node totals, and network connectivity
...
**Running CONCOCT and Contig Enabled Models (CEMs) Definition**
clusters=400
...
**Figure 4 | ** 
**(a)** Single interaction algorithm diagram
**(b)** Community-level algorithm diagram
**(c)** C. difficile and Lactobacillus CEM in cef-pretreatement example
**(d)** Infected cef-pretreated community for demand and most likely produced metabolites
...
**Supplementary Figure 4 | Impact of antibiotic pretreatment and *C. difficile* infection on cecal metabolome** NMDS ordinations of Bray-Curtis distances comparing the dissimilarity of pretreatment and infection groups. **(a)** Antibiotic-pretreated compared to Untreated controls and **(b)** comparison between only antibiotic pretreatment groups. **(c-e)** Within antibiotic pretreatment comparisons for the effect of *C. difficile* colonization on community structure. Significant differences and correlation coefficients for ordination analyses were found using ANOSIM.
