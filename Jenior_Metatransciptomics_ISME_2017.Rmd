...
In order to evaluate changes in bacterial population abundances in response to perturbation and *C. difficile* colonization, we performed 16S rRNA gene V4 region sequencing from cecal content of both mock and *C. difficile*-infected mice across antibiotic pretreatment models. To maintain focus on the surrounding bacterial communities, we removed all sequences contributed by *C. difficile* itself before proceeding with downstream analysis. First, we reconfirmed that antibiotic treatment significantly impacted both cecal community structure and inverse-Simpson diversity compared to untreated controls (all *p* < 0.001; Fig. S1a & S1b). We then moved on to assess that the different antibiotic classes each induced distinct community structures that all significantly differed from untreated mice and one another (Fig. S1c). Cefoperazone and clindamycin-pretreated cecal were consistently dominated by the families Lactobacillaceae and Enterobacteriaceae respectively (Fig. 1c). However, only clindamycin allowed for significant change in structure following *C. difficile* colonization (*p* < 0.001; Fig. S1c & S1d). Streptomycin-pretreated communities were more variable between cages or individuals but were generally enriched for members of phylum Bacteroidetes but did significantly shift upon infection (Fig. S1e). Next, we identified which specific OTUs significantly differed by abundance between mock and *C. difficile* infection within each pretreatment group. To accomplish this, we performed paired Wilcoxon rank-sum tests on the relative abundance of each OTU in either *C. difficile* or mock-infected cecal communities which revealed a total of 13 OTUs that significantly changed in abundance in at least one pretreatment group following infection (Fig. 1d-1f). The most frequent difference in the abundance of OTUs between mock and infected conditions was that sequences for the given OTU were only detectable in one of the two groups. Furthermore, the direction of change was not consistent across all OTUs in that some would increase following *C. difficile* colonization while others instead decreased. The majority (9 of 13) of the identified OTUs increased in the presence of *C. difficile* including an Escherichia OTU, which this groups has been previously demonstrated to synergize with *C. difficile* under certain circumstances [@]. Only 4 OTUs were shown to significantly decrease following infection including an Akkermansia OTU which in a previous study had been associated with ... [@]. Lactobacillus (OTU1) was the only OTU which was significantly altered in multiple antibiotic pretreatments (cefoperazone & clindamycin). These results also indicate that certain taxonomic groups may be more effected by *C. difficile* colonization than others through possible synergy or antagonism.
...
**Figure 1 | Experimental models of *C. difficile* infection and their impact on bacterial populations in the gut microbiota.** **(a)** Experimental timelines of antibiotic pretreatment and infection mouse models. **(b)** Quantification of *C. difficile* cfu and toxin titer across antibiotic pretreatment models. Gray asterisks indicate significant difference from no antibiotic controls by Wilcoxon rank-sum test with the Benjamini-Hochberg correction (all *p* < 0.001). **(c)** Relative abundance of family-level taxonomic classification for OTUs in each treatment group. Specific OTUs and genus-level classifications from LEfSe analysis that distinguish Mock and *C. difficile*-infected communities within each pretreatment group (all *p* < 0.05). Relative abundance of the respective OTUs in each mouse along with group medians are shown; **(d)** streptomycin, **(e)** cefoperazone, and **(f)** clindamycin. 

**Figure 2 | *C. difficile* infection alters specific pathway and gene expression of the gut microbiota across perturbed communities** Metatranscriptomic (cDNA) read abundances associated with each gene were normalized to their associated metganomic (DNA) coverage, resulting in values that reflect upregulation and not basal expression levels of highly abundant genes. **(a)** Differences in read abundance for top 5 KEGG pathways with the largest amount of change between Mock and *C. difficile*-infected states within each antibiotic pretreatment. Values above the center line were expressed more during *C. difficile* infection, and those below the line were expressed more in Mock-infected animals. **(b-d)** cDNA read abundances of the top 15 genes with the largest differences in expression within each indicated antibiotic pretreatment group. Shown are the actual expression levels for the genes displayed on the left during Mock (white) or *C. difficile* (black) infection. Abundances for each annotated pathway or gene were reported at the community-level and were agnostic to individual taxonomic group contributions to transcript pool.
