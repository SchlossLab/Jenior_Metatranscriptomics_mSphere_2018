...
We then moved on to a more fine-scale resolution analysis and quantified expression changes at the individual gene annotation level. Similar to the pathway-level analysis, transcription for gene homologs between species was pooled to find those genes with the greatest overall difference in expression between conditions. We then selected the genes in each antibiotic pretreatment group with the largest disparity in normalized cDNA abundance between mock and *C. difficile*-infected groups (Fig. 2). In agreement with pathway-level differences in expression, for streptomycin pretreatment the genes with ... (Fig. 2b). Along similar lines reflecting results of pathway expression, the metatranscriptome in cefoperazone pretreatment ... (Fig. 2c). It important to note that these groups are equally subsampled, meaning that more genes are evenly expressed at lower levels in mock infection instead of a small number of genes highly upregulated. Alternatively, in clindamycin pretreatment ... (Fig. 2d). These data expand on our interpretation of pathway-level analysis in that *C. difficile* colonization leads to changes in community-level expression of genes for acquisition and catabolism. Our results also raised questions about which taxonomic groups underscore the differences seen and are potentially more impacted by *C. difficile* colonization than others.
