...
**Figure 1 | Experimental models of *C. difficile* infection and their impact on bacterial populations in the gut microbiota.** **(a)** Experimental timelines of antibiotic pretreatment and infection mouse models. **(b)** Quantification of *C. difficile* cfu and toxin titer across antibiotic pretreatment models. Gray stars indicate significant difference from no antibiotic controls by Wilcoxon rank-sum test (all *p* < 0.001). **(c)** Relative abundance of family-level taxonomic classification for OTUs in each treatment group. Pair-wise Wilcoxon tests were performed for each OTU between mock and *C. difficile*-infected conditions within antibiotic pretreatment group; **(d)** streptomycin, **(e)** cefoperazone, and **(f)** clindamycin. All p-values have been corrected with the Benjamini-Hochberg procedure.


**Figure 2 | Community-level gene expression** 

**Figure 3 | *C. difficile* colonization alters community-level gene expression differently between antibiotic pretreatements.** Each point represents a unique gene from the respective metagenomic assembly. Coordinates determined by the log~2~-transformed expression level of each gene between *C. difficile*-infected and mock-infected conditions. Metatranscriptomic read abundances are normalized to their associated metganomic coverage. Colored indicate genus of origin. Gray bars were determined utilizing least squares regression analysis with a minimum residual value of 2. Expreimental groups are shown as follows; **(a)** Streptomycin-pretreated, **(b)** Cefoperazone-pretreated, and **(c)** Clindamycin-pretreated. 
